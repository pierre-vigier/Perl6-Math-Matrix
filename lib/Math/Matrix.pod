...
All operators listed below (except MM) will be exported automatically.
Acceptable export tags are :MANDATORY (nothing is exported), :DEFAULT (same as no tag, most ops), :MM (only MM), :ALL.
...
=item accessors: cell, row, column, diagonal, submatrix, AT-POS
=item conversion: Bool, Numeric, Str, Array, Hash, list, list-rows, list-columns, gist, perl
