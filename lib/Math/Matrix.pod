...
=item conversion: Bool, Numeric, Str, Array, list, list-rows, list-columns, gist, perl
...
=item numeric properties: cell-type, size, density, trace, determinant, rank, kernel, norm, condition
...
=item list like ops: elems, cont, equal, map, map-row, map-column, map-cell, reduce, reduce-rows, reduce-columns
...
All values separated by one whitespace, rows by new line.
It is called implicitly by put and print. A shortened version is provided by .gist
Same as the input of new.
...
Several dots will hint that something is missing. It is implicitly called by say.
For a full view use .Str
...
Matrix cells can be of type (Bool), (Int), (Rat), (Num) or (Complex). 
The widest type of any cell will returned as type object.
In this case you get true, because no cell of our default matrix has wider type
than (Int) and cells can be safely treated as such.
