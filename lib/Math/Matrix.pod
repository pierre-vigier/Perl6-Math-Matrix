...
=item B<L<constructors|#constructors>>: L<new []|#new-->, L<new ()|#new---1>, L<new ""|#new---2>,
      L<new-zero|#new-zero>, L<new-identity>, L<new-diagonal>, L<new-vector-product>
=item B<L<accessors|#accessors>>: L<element>, L<AT-POS|#at-pos>, L<row>, L<column>, L<diagonal>, L<skew-diagonal>, L<submatrix>
=item B<L<converter|#converter>>: L<Bool|#bool>, L<Str|#str>, L<Numeric|#numeric>, L<Range|#range>,
      L<Array|#array>, L<list>, L<list-rows>, L<list-columns>, L<Hash|#hash>, L<gist>, L<perl>
=item B<L<boolean properties|#boolean-properties>>: L<square|#is-square>, L<triangular|#is-triangular>, L<frobenius|#is-frobenius>,
      L<zero|#is-zero>, L<identity|#is-identity>, L<diagonal|#is-diagonal>, L<-dominant|#is-diagonally-dominant>, L<-constant|#is-diagonal-constant>, L<catalecticant|#is-catalecticant>,
...
=item B<L<numeric properties|#numeric-properties>>: L<size>, L<density>, L<bandwith>, L<trace>, L<rank>, L<nullity>, L<determinant>, L<minor>, L<norm>, L<condition>, L<element-type>
=item B<L<derived matrices|#drived-matrices>>: L<transposed>, L<negated>, L<conjugated>, L<adjugated>, L<inverted>, L<reduced-row-echelon-form>
=item B<L<decompositions|#decompositions>>: L<LU|#decompositionlu>, L<LUCrout|#decompositionlucrout>, L<Cholesky|#Cholesky-decomposition>
=item B<L<math ops|#mathematical-operations>>: L<equal>, L<add>, L<multiply>, L<dot-product>, L<tensor-product>
=item B<L<list like ops|#list-like-operations>>: L<elems>, L<elem>, L<cont>, L<map>, L<map-with-index>,
      L<reduce>, L<reduce-rows>, L<reduce-columns>
=item B<L<structural ops|#structural-operations>>: L<move-row>, L<move-column>, L<swap-rows>, L<swap-columns>, L<splice-rows>, L<splice-columns>
...
as checked by L<is-zero>. It has the L<size> as given by arguments.
If only one argument is given, the matrix is L<quadratic|#is-square>. All the L<element>s are set to 0.
...
This method is a constructor that returns an identity matrix (as checked by L<is-identity>)
of the size given in the only and required parameter.
All the L<element>s are set to 0 except the top/left to bottom/right diagonale is set to 1.
...
This method is a constructor that returns an diagonal matrix (as checked by L<is-diagonal>)
of the size given by count of the parameter.
All the L<element>s are set to 0 except the top/left to bottom/right diagonal, set to given values.
...
the matrix product (method L<dot-product>, or operator dot) of a column vector
...
L<element>, L<AT-POS|#at-pos>, L<row>, L<column>, L<diagonal>, L<skew-diagonal>, L<submatrix>
...
See: L<elems>, L<elem>, L<element-type>
...
a (main) skew diagonal is only defined for L<square|#is-square> matrixes and runs
from left lower corner to the right upper (from $matrix[n][0] to $matrix[0][n]).
...
L<Range|#range>, L<Array|#array>, L<Hash|#hash>, L<list>, L<list-rows>, L<list-columns>
or allow different views on the overall content (output formats): L<gist>, L<perl>.

=head3 L<Bool|#converter>

Conversion into Bool context. Returns False if matrix is zero
(all elements equal zero as in L<is-zero>), otherwise True.
...
Returns values of all L<element>s, separated by one whitespace, rows by new line.
This is the same format as expected by L<new("")|#new---2>.
Str is called implicitly by put and print. A shortened version is provided by L<gist>
...
Conversion into Numeric context. Returns Euclidean L<norm>.
...
Returns an range object that reflects the content of all L<element>s.
...
Content of all L<element>s as an array of arrays (same format that was put into L<new([...])|#new-->).
...
Returns a flat list with all L<element>s (same as .list-rows.flat.list).
...
Arguments, like in case of L<is-diagonally-dominant>,
are only necessary when a method can tell you about a group of closely related properties.

L<square|#is-square>, B<L<triangular|#is-triangular>>, L<frobenius|#is-frobenius>,
L<zero|#is-zero>, L<identity|#is-identity>, L<diagonal|#is-diagonal>,
B<L<diagonally-dominant|#is-diagonally-dominant>>, L<diagonal-constant|#is-diagonal-constant>, L<catalecticant|#is-catalecticant>,
...
True if all L<element>s above or below the main L<diagonal> are zero.
This method accepts four optional, boolean arguments:
L<:upper|#upper-triangular>, L<:lower|#lower-triangula>, L<:strict|#strict-triangular> and L<:unit|#unit-triangular>.
...
Please note that a triangular matrix can never be I<:unit> and I<:strict> at the same time,
nor I<:!upper> and I<:!lower>. A triangular matrix that is I<:upper> and I<:lower> L<is-diagonal>.
L<Identity|#is-identity> matrices are I<:upper>, I<:lower> and I<:unit>.

=head4 L<upper triangular|#is-triangular>

a.k.a I<right triangular matrix>: all L<element>s left-below the main I<diagonal> are zero.
In other words: the L<lower-bandwith> has to be zero.
...
a.k.a I<left triangular> matrix: every L<element> right and above the L<diagonal>
(where row index is greater than column index) are zero.
In other words: the L<upper-bandwith> has to be zero.
...
are triangular matrices that have a L<diagonal> consisting only of values equal one.
...
are triangular matrices that have a L<diagonal> consisting only of values equal zero.
...
=head3 L<is-frobenius|#boolean-properties>

L<Square|#is-square> matrix which differs from L<identity|#is-identity> only in the lower part (below diagonal)
of one column. (Main diagonal consist of ones and rest is zero.)
...
True if every L<element> has value of 0 (as created by L<new-zero|#new-zero>).

=head3 L<is-identity|#boolean-properties>

True if every L<element> on the main L<diagonal> (where row index equals column index) is 1
...
True if matrix is L<square|#is-square> and only elements on the L<diagonal> differ from 0.
...
True when L<element>s on the L<diagonal> have a bigger (if strict) or at least equal (in none strict)
...
True if every L<diagonal> is the a collection of L<element>s that hold the same value.
...
True if every L<element> with coordinates x y has same value as the element on y x.
...
L<determinant|#determinant>, L<minor|#minor>, L<norm>, L<condition|#condition>, B<L<element-type|#element-type>>:
...
I<Density> is the percentage of L<element>s which are not zero. I<sparsity> = 1 - I<density>.

    my $d = $matrix.density;

=head3 L<bandwith|#numeric-properties>

Is roughly the greatest distance of a none zero value from the main L<diagonal>.
...
In a matrix with the lower bandwith of k, every L<element> with row index m and column index n,
for which holds m - k > n, the content has to be zero. Literally speaking: there are k L<diagonal>s
below the main diagonal, that contain none zero values.

=head4 L<upper-bandwith|#bandwith>

Analogously, every L<element> with m + k < n is zero if matrix has an upper bandwith of k.
...
The trace of a L<square|#is-square> matrix is the sum of the L<element>s on the main diagonal.
...
The two required positional arguments are row and column indices of an existing L<element>.
...
the absolute value of every L<element> is taken to the power of p.
...
Matrix L<element>s can be (from most narrow to widest), of type (Bool), (Int), (Num), (Rat), (FatRat) or (Complex).
...
Creates a matrix where every L<element> has the negated value of the original (invertion of sign).
...
Creates a matrix where every L<element> is the complex conjugated of the original.
...
L<LU-decomposition|#decompositionLU>,
L<LU-decomposition-Crout|#decompositionLUCrout>,
L<Cholesky-decomposition|#Cholesky-decomposition>
...
This decomposition does roughly the same and is faster than the previous,
but works only on matrices that are L<symmetric|#is-symmetric> and L<positive-definite|#is-positive-definite>.
The result will be a L<lower triangular matrix|#is-lower-triangular> matrix (here called G)
that multiplied with its L<transposed|#transposed> gives you the original matrix.
When the optional, boolean parameter :diagonal is positive (negative is default)
you get two matrices (L and D) as a result. L again is a L<lower triangular matrix|#is-lower-triangular>,
but with ones in its main diagonal. D is a L<diagonal> matrix ( G = L * sqrt(D)).
...
Checks two matrices for equality. They have to be of same L<size|#size> and every L<element>
...
When adding a single number to the matrix, it will be added to every L<element>.
...
this operation is the simple, scalar multiplication applied to L<element>s.
...
C is a concatination of matrices you get if you take every L<element> of A
...
Number (count) of L<element>s = rows * columns (see L<size|#size>).

    say $matrix.elems();

=head3 L<elem|#list-like-operations>

Asks if all L<element> of Matrix (cell) values are an element of the given set or range.
...
Creates a new matrix of same size by iterating over all or some L<element>s.
...
Like the built in reduce method, it iterates over all L<element>s and joins
