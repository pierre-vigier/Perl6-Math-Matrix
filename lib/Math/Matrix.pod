...
If only one argument is given, the matrix is L<quadratic|#is-square>. All the elements are set to 0.
...
All the elements are set to 0 except the top/left to bottom/right diagonale is set to 1.
...
All the elements are set to 0 except the top/left to bottom/right diagonal, set to given values.
...
(all elements equal zero as in is-zero), otherwise True.
...
Returns values of all elements, separated by one whitespace, rows by new line.
...
Returns an range object that reflects the content of all elements.
...
Content of all elements as an array of arrays (same format that was put into L<new([...])|#new-->).
...
True if every element has value of 0 (as created by L<new-zero|#new-zero>).

=head3 L<is-identity|#boolean-properties>

True if every element on the main L<diagonal|#diagonal> (where row index equals column index) is 1
...
True if every element below the L<diagonal|#diagonal> (where row index is greater than column index) is 0.
...
True if every element above the L<diagonal|#diagonal> (where row index is smaller than column index) is 0.
...
True when elements on the L<diagonal|#diagonal> have a bigger (if strict) or at least equal (in none strict)
...
True if every L<diagonal|#diagonal> is the a collection of elements that hold the same value.
...
True if every element with coordinates x y has same value as the element on y x.
...
I<Density> is the percentage of element which are not zero. I<sparsity> = 1 - I<density>.
...
In a matrix with the lower bandwith of k, every element with row index m and column index n,
...
The trace of a L<square|#is-square> matrix is the sum of the elements on the main diagonal.
...
Arguments are row and column of an existing L<element|#element>.
...
the absolute value of every element is taken to the power of p.
...
Matrix elements can be (from most narrow to widest), of type (Bool), (Int), (Num), (Rat), (FatRat) or (Complex). 
...
When adding a single number to the matrix, it will be added to every element.
...
this operation is the simple, scalar multiplication applied to elements.
...
Like the well known I<map> function that works on lists,
this method iterates over all or some elements of the matrix.
Each time a code block (only required argument) will be called with the that gets the element value as argument.
...
Runs a code block (only required argument) for every element of the matrix.
Arguments to the anonymous block are current row and column index 
and the content of the element. The results for a new matrix.
