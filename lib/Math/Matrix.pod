...
Matrices are tables with rows and columns (index counting from 0) of numbers (Numeric type): 
 ...
 transpose, invert, negate, add, multiply, dot product, tensor produc, determinant, 
 rank, trace, norm, decompositions, append, map, reduce and so on
...
This method is a constructor that returns an empty matrix of the size given in parameter.
...
=head3 prepend-vertically

Both variants work equally and also all other prepend/append operations. 
They can not be combined and work only if proper matrix dimensions do match.
