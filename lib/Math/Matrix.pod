...
This decomposition is faster than the previous, but works only on matrices that are
L<symmetric|#is-symmetric> and L<positive-definite|#is-positive-definite>.
Four output formats are supported: G (default), GG, LD and LDL
(GG and LDL are just convenience). The first G or L in a format name refers to a
L<lower triangular matrix|#is-lower-triangular> and D to a L<diagonal|#is-diagonal> matrix.
The main L<diagonal|#diagonal> of L is filled with ones and G = L * sqrt(D).
In each case the second L or G is the L<transposed|#transposed> version of the first G or L.
