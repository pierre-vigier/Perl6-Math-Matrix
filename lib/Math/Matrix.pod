...
They are: L<equal|#equal>, B<L<add|#add>>: (L<matrix|#add-matrix>, L<vector|#add-vector>, L<scalar|#add-scalar>),
...
Adding a matrix, L<vector|#add-vector> or L<add-scalar|#scalar>.
...
this operation is the simple, scalar multiplication applied to each L<element|#element>.
