...
=item L<constructors|#constructors>: L<new []|#new-->, L<new ()|#new---1>, L<new ""|#new---2>,
...
=item L<converter|#type-conversion-and-output-formats>: L<Bool|#bool>, L<Numeric|#numeric>, L<Str|#str>,
      L<Array|#array>, L<Hash|#hash>, L<Range|#range>, L<list|#list>, L<list-rows|#list-rows>, L<list-columns|#list-columns>, L<gist|#gist>, L<perl|#perl>
=item L<boolean properties|#boolean-properties>: L<zero|#is-zero>, L<identity|#identity>, L<square|#is-square>, L<diagonal|#is-diagonal>, L<diagonal-constant|#is-diagonal-constant>, 
      L<diagonally-dominant|#is-diagonally-dominant>, L<upper-triangular|#is-upper-triangular>, L<lower-triangular|#is-lower-triangular>, L<invertible|#is-invertible>,
      L<symmetric|#is-symmetric>, L<antisymmetric|#is-antisymmetric>, L<unitary|#is-unitary>, L<self-adjoint|#is-self-adjoint>, L<orthogonal|#is-orthogonal>,
      L<positive-definite|#positive-definite>, L<positive-semidefinite|#is-positive-semidefinite>
=item L<numeric properties|#numeric-properties>: L<size|#size>, L<density|#density>, L<trace|#trace>, L<determinant|#determinant>, L<rank|#rank>, L<nullity|#nullity>,
      L<norm|#norm>, L<condition|#condition>, L<minor|#minor>, L<narrowest-|#narrowest-cell-type>, L<widest-cell-type|#widest-cell-type>
=item L<derived matrices|#derived-matrices>: L<transposed|#transposed>, L<negated|#negated>, L<conjugated|#conjugated>, L<adjugated|#adjugated>, L<inverted|#inverted>,
...
=item L<matrix math ops|#matrix-math-operations>: L<equal|#equal>, L<add|#add>, L<multiply|#multiply>, L<dot-product|#dot-product>, L<tensor-product|#tensor-product>
=item L<list like ops|#list-like-matrix-operations>: L<elems|#elems>, L<elem|#elem>, L<cont|#cont>,
      L<map-index|#map-index>, L<map-with-index|#map-with-index>, L<map|#map>, L<map-row|#map-row>, L<map-column|#map-column>,
      L<reduce|#reduce>, L<reduce-rows|#reduce-rows>, L<reduce-columns|#reduce-columns>
=item L<structural ops|#structural-matrix-operations>: L<move-row|#move-row>, L<move-column|#move-column>, L<swap-rows|#swap-rows>, L<swap-columns|#swap-columns>,
      L<splice-rows|#splice-rows>, L<splice-columns|#splice-columns>
=item L<shortcuts|#shortcuts>:   L<T|#transposed>,   L<conj|#conjugated>,   L<det|#determinant>,   L<rref|#reduced-row-echelon-form>
=item L<operators|#operator-methods>:   MM,  ?,  ~,  |,  @,  %,  +,  -,  *,  **,  dot,  ⋅,  ÷,  X*,  ⊗,  ==,  ~~,  ❘ ❘,  ‖ ‖,  [ ]
...
Matrix does not have to be a quadratic (square).
...
If you provide two ranges (row-min .. row-max, col-min .. col-max) to the appropriately named arguments,
you get the two dimensional excerpt of the matrix that is defined by these ranges.

    say $m.submatrix( rows => 1..1, columns => 0..*) :    3 4 5        

When provided with two lists (or arrays) of values (to the arguments named "rows" and "columns")
...
The named arguments of both types can be mixed and are in both cases optional.
...
=head3 L<Range|#type-conversion-and-output-formats>
...
True if every cell below the L<diagonal|#diagonal> (where row index is greater than column index) is 0.
...
True if every cell above the L<diagonal|#diagonal> (where row index is smaller than column index) is 0.
...
True if matrix is L<square|#is-square> and only cells on the L<diagonal|#diagonal> differ from 0.
...
Checks if caller is a diagonal-constant or Töplitz matrix.
True if every L<diagonal|#diagonal> is the a collection of cells that hold the same value.
...
True when cells on the L<diagonal|#diagonal> have a bigger (if strict) or at least equal (in none strict)
...
A Hermitian or self-adjoint matrix is equal to its L<transposed|#transposed> and complex L<conjugated|#conjugated>.
...
Is True if number of rows and colums are the same (L<is-square|#is-square>) and L<determinant|#determinant> is not zero.
...
is an identity matrix or in other words: L<transposed|#transposed> and L<inverted|#inverted> matrices are equal.
...
I<Density> is the percentage of cell which are not zero. I<sparsity> = 1 - I<density>.
...
Arguments are row and column of an existing cell.
A Minor is the determinant of a L<submatrix|#submatrix> (2 scalar argument, first variant).
...
Matrices that have a square form and a full rank can be inverted (see L<is-invertible|#is-invertible>).
Inverse matrix regarding to matrix multiplication (see L<dot-product|#dot-product>).
The dot product of a matrix with its inverted results in a L<identity|#is-identity> matrix
...
=head3 L<equal|#matrix-math-operations>
...
(on the same position) of the second matrix (pass a check with ==).
...
=head4 L<add matrix|#matrix-math-operations>

When adding two matrices, they have to be of the same size. Instead of
Math::matrix object you can also provide the cell data as 
...
=head4 L<add vector|#matrix-math-operations>
...
=head4 L<add scalar|#matrix-math-operations>

When adding a single number to the matrix, it will be added to every cell.
If you provide a row or column number it will be only added to that row or column.
In case you provide both, only a single cell gets a different value in the result matrix.
...
this operation is the simple, scalar multiplication applied to cells.
...
of that size again. Each cell will be the product of two the two cells of the 
...
=head3 L<dot-product|#matrix-math-operations>
...
=head3 L<tensor-product|#matrix-math-operations>

The tensor product (a.k.a Kronecker product) between a matrix a of size (m,n)
and a matrix b of size (p,q) is a matrix c of size (m*p,n*q).
All matrices you get by multiplying an element (cell) of matrix a with
 matrix b (as in $a.multiply($b.cell(..,..)) concatinated result in matrix c. 
...
=head3 L<map-index|#list-like-matrix-operations>

Runs a code block (only required argument) for every cell of the matrix.
Arguments to the anonymous block are current row and column index.
...
=head3 L<map-with-index|#list-like-matrix-operations>

Runs a code block (only required argument) for every cell of the matrix.
Arguments to the anonymous block are current row and column index 
and the content of the cell. The results for a new matrix.
...
Like the built in map it iterates over all elements (cell values), 
running a code block (only required argument) that gets the cell value as argument.
The results build a new matrix.
...
Operators with method aliases, for more explanations loo into L<ops chapter|#operators>:
...
The Module overloads or introduces a range of well and lesser known ops.
==, +, * are commutative, -, ⋅, dot, ÷, x, ⊗ and ** are not.

They are exported when using no flag or under the export flags :DEFAULT or :ALL,
but not under :MANDATORY or :MM).
