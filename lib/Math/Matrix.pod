...
=item B<L<list like ops|#list-like-operations>>: L<elems|#elems>, L<elem|#elem>, L<cont|#cont>,
        L<map|#map>, L<map-with-index>, L<reduce|#reduce>, L<reduce-rows|#reduce-rows>, L<reduce-columns|#reduce-columns>
...
creates a L<diagonal|#is-diagonal> matrix which again contains zeros, except in the
main L<diagonal|#diagonal>, that can hold arbitrary values. The only required argument
is a list of numbers, which will become the content of the main diagonal.
Naturally diagonal matrices are also always L<quadratic|#is-square>.
...
L<element|#element>, L<AT-POS|#at-pos>, L<row|#row>, L<column|#column>, L<diagonal|#diagonal>, L<skew-diagonal>, L<submatrix>
...
Without an argument it returns the values of main diagonal elements as a list.
The main diagonal runs from the left upper corner and going in each step
one cell down and one right the reach the next element of the main diagonal.
...
In mathematics, a submatrix is built by leaving out one L<row|#row> and one column.
...
or allow different views on the overall content (output formats): L<gist|#gist>, L<perl|#perl>.
...
L<symmetric|#is-symmetric>, L<anti-symmetric|#is-antisymmetric>, L<unitary|#is-unitary>,
L<self-adjoint|#is-self-adjoint>, L<invertible|#is-invertible>, L<orthogonal|#is-orthogonal>,
L<positive-definite|#is-positive-definite>, L<positive-semidefinite|#is-positive-semidefinite>
...
True if all L<element|#element>s above or below the main L<diagonal|#diagonal> are zero.
...
nor I<:!upper> and I<:!lower>. A triangular matrix that is I<:upper> and I<:lower> L<is-diagonal|#is-diagonal>.
...
are triangular matrices that have a L<diagonal|#diagonal> consisting only of values equal one.
...
are triangular matrices that have a L<diagonal|#diagonal> consisting only of values equal zero.
...
but with ones in its main diagonal. D is a L<diagonal|#diagonal> matrix ( G = L * sqrt(D)).
