...
Matrix cells can be of type (Int), (Rat), (Num) or (Complex). 
...
The only exception is MM, a shortcut to create a matrix. That has to be importet
explicitly with the tag :MM or :ALL.
